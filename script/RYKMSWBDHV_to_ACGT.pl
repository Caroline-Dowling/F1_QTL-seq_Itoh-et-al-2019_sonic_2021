...
sub scan_pileup_and_convert() {
	...
	open(PILEUParent_A, $pileup_file1) or die "No $pileup_file1";
	open(PILEUParent_B, ">$pileup_file2") or die "No $pileup_file2";
	...
	while (<PILEUParent_A>) {
		...
		if ($cons =~ /^[ACGTN]$/) {
			...
			# printf PILEUParent_B "%s\t%d\t%s\t%s\t%d\t%d\t%d\t%d\t%s\t%s\t%.2f\n", $chr, $pos, $ref, $cnvcons, $q1, $q2, $q3, $depth, $bases, $quals, $snpindex;

			# printf STDERR  "%s\t%d\t%s\t%s\t%d\t%d\t%d\t%d\t%s\t%s\t%s\n", $chr, $pos, $ref, $cnvcons, $q1, $q2, $q3, $depth, $bases, $quals, $snpindex;
			printf PILEUParent_B "%s\t%d\t%s\t%s\t%d\t%d\t%d\t%d\t%s\t%s\t%s\n", $chr, $pos, $ref, $cnvcons, $q1, $q2, $q3, $depth, $bases, $quals, $snpindex;
		}
		# next if ($ref =~ /^\*$/);
	}
